...
Shader "Unlit/Hatching"
{
	Properties
	{
		//textures: all 6 TAM images in 2 textures (RGB channels) -> only two texture accesses
		...
		SubShader
	{
		...
		Pass
		{
			...
			fixed3 HatchingConstantScale(float2 _uv, half _intensity, float _dist) // _dist is distance from camera, multiplied by unity_CameraInvProjection[0][0]
			{
				...
				half3 weightsA = saturate((_intensity * 18.0) + half3(-0, -1, -2));
				half3 weightsB = saturate((_intensity * 18.0) + half3(-3, -4, -5));
